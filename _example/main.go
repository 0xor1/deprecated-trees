...
import(
	...
	"bitbucket.org/robsix/task_center/central_directory/api/user"
	"fmt"
)

func main(){
	log := zap.New(zap.NewTextEncoder(), zap.DebugLevel)
	api, _ := user.NewApi(nil, nil, nil, []string{`^\w*$`}, []string{`[0-9]`, `[a-z]`, `[A-Z]`, `[\W]`}, 3, 100, 1, 30, 8, 20, 40, 128, 16384, 8, 1, 32, log)
