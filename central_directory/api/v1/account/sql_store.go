...
func (s *sqlStore) createUser(user *fullUserInfo, pwdInfo *pwdInfo) {
	...
	if _, err := s.pwdsDB.Exec(query_createUser_pwds, id, pwdInfo.salt, pwdInfo.pwd, pwdInfo.n, pwdInfo.r, pwdInfo.p, pwdInfo.keyLen); err != nil {
		panic(err)
	}
}
...
func (s *sqlStore) updateUser(user *fullUserInfo) {
	id := []byte(user.Id)
	if _, err := s.accountsDB.Exec(query_updateUser, id, user.Name, user.CreatedOn, user.Region, user.NewRegion, user.Shard, user.HasAvatar, user.Email, user.NewEmail, user.activationCode, user.activated, user.newEmailConfirmationCode, user.resetPwdCode); err != nil {
		panic(err)
	}
}
...
func (s *sqlStore) updateAccount(account *account) {
	if _, err := s.accountsDB.Exec(query_updateAccount, account.Name, account.CreatedOn, account.Region, account.NewRegion, account.Shard, account.HasAvatar, account.IsUser, []byte(account.Id)); err != nil {
		panic(err)
	}
}
...
func (s *sqlStore) updatePwdInfo(id Id, pwdInfo *pwdInfo) {
	if _, err := s.pwdsDB.Exec(query_updatePwdInfo, pwdInfo.salt, pwdInfo.pwd, pwdInfo.n, pwdInfo.r, pwdInfo.p, pwdInfo.keyLen, []byte(id)); err != nil {
		panic(err)
	}
}
...
func (s *sqlStore) deleteAccountAndAllAssociatedMemberships(id Id) {
	castId := []byte(id)
	if _, err := s.accountsDB.Exec(query_deleteAccountAndAllAssociatedMemberships_accounts, castId); err != nil {
		panic(err)
	}
	if _, err := s.pwdsDB.Exec(query_deleteAccountAndAllAssociatedMemberships_pwds, castId); err != nil {
		panic(err)
	}
}
...
func (s *sqlStore) createOrgAndMembership(org *account, user Id) {
	if _, err := s.accountsDB.Exec(query_createOrgAndMembership, []byte(org.Id), org.Name, org.CreatedOn, org.Region, org.NewRegion, org.Shard, org.HasAvatar, false, []byte(user)); err != nil {
		panic(err)
	}
}
