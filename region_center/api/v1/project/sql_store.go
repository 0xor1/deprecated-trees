...
func (s *sqlStore) setIsParallel(shard int, accountId, projectId Id, isParallel bool) {
	s.shards[shard].Exec(`UPDATE projects SET isParallel=? WHERE account=? AND id=?`, isParallel, []byte(accountId), []byte(projectId))
}
...
func (s *sqlStore) getPublicProjects(shard int, accountId Id, nameContains *string, createdOnAfter, createdOnBefore, startOnAfter, startOnBefore, dueOnAfter, dueOnBefore *time.Time, archived bool, sortBy SortBy, sortDir SortDir, offset, limit int) ([]*project, int) {
	...
	query := bytes.NewBufferString(`SELECT %s FROM projects WHERE account=? AND isPublic=true`)
	...
	query.WriteString(` ORDER BY ? ? LIMIT ? OFFSET ?`) //not sure if this syntax works with order by clause, might need to use fmt.Sprintf to inject sortBy and sortDir values
