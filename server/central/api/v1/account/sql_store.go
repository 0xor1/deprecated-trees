...
func (s *sqlStore) searchAccounts(nameOrDisplayNameStartsWith string) []*account {
	...
	rows, err := s.accounts.Query(`SELECT id, name, displayName, createdOn, region, newRegion, shard, hasAvatar, isPersonal FROM accounts WHERE name LIKE ? OR displayName LIKE ? ORDER BY name ASC LIMIT ?, ?`, searchTerm, searchTerm, 0, 100)
	if rows != nil {
		defer rows.Close()
	}
	PanicIf(err)
...
func (s *sqlStore) searchPersonalAccounts(nameOrDisplayNameOrEmailStartsWith string) []*account {
	...
	//TODO need to profile these queries to check for best performance
	rows, err := s.accounts.Query(`SELECT id, name, displayName, createdOn, region, newRegion, shard, hasAvatar FROM personalAccounts WHERE name LIKE ? OR displayName LIKE ? OR email LIKE ? ORDER BY name ASC LIMIT ?, ?`, searchTerm, searchTerm, searchTerm, 0, 100)
	if rows != nil {
		defer rows.Close()
	}
	PanicIf(err)
