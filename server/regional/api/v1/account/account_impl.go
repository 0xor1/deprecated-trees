...
func (a *api) GetActivities(shard int, accountId, myId Id, itemId *Id, memberId *Id, occurredAfter, occurredBefore *time.Time, limit int) []*Activity {
	if occurredAfter != nil && occurredBefore != nil {
		InvalidArgumentsErr.Panic()
	}
	ValidateMemberHasAccountAdminAccess(a.store.getAccountRole(shard, accountId, myId))
	return a.store.getActivities(shard, accountId, itemId, memberId, occurredAfter, occurredBefore, ValidateLimitParam(limit, a.maxProcessEntityCount))
}
...
type store interface {
	...
	getActivities(shard int, accountId Id, item *Id, member *Id, occurredAfter, occurredBefore *time.Time, limit int) []*Activity
}
