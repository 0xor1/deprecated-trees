...
func Test_system(t *testing.T) {
	...
	aNote := "word up!"
	tl1, err := client.Create(aliCss, 0, org.Id, proj.Id, taskD.Id, 30, &aNote)
	assert.Nil(t, err)
	assert.True(t, tl1.Project.Equal(proj.Id))
	assert.True(t, tl1.Member.Equal(aliId))
	assert.True(t, tl1.Task.Equal(taskD.Id))
	...
	tl2, err := client.CreateAndSetRemainingTime(bobCss, 0, org.Id, proj.Id, taskD.Id, 100, 30, &aNote)
	assert.Nil(t, err)
	assert.True(t, tl2.Project.Equal(proj.Id))
	assert.True(t, tl2.Member.Equal(bobId))
	assert.True(t, tl2.Task.Equal(taskD.Id))
	...
	assert.Nil(t, taskClient.Delete(aliCss, 0, org.Id, proj.Id, taskD.Id))
